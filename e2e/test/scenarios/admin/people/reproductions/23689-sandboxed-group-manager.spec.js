...
describeEE("issue 23689", (() => {
  ...
  test('sandboxed group manager should see all other members (metabase#23689)', async ({ page }) => {
    visitGroupPermissionsPage(COLLECTION_GROUP);
...
async function findUserByFullName(user) {
  const { first_name, last_name } = user;
  return cy.findByText(`${first_name} ${last_name}`);
}

async function visitGroupPermissionsPage(groupId) {
