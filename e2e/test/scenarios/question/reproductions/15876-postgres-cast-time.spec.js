...
test.describe("issue 15876", { tags: "@external" }, () => {
  test.beforeEach(async () => {
    restore("postgres-12");
    cy.signInAsAdmin();
  });

  test("should correctly cast to `TIME` (metabase#15876)", async ({ page }) => {
    ...
    await page.locator(".Visualization").within(async () => {
      for (const { value, rows } of correctValues) {
        ...
        await expect(page.locator(`:text("${value}")`)).toHaveCount(count);
      }
    });
  });
});
