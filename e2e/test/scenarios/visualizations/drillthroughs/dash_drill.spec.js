  ...
  test.describe("card title click action", () => {
    ...
    test.describe("from a scalar card", () => {
      ...
      test.beforeEach(async () => {
        ...
        await request("PUT", `/api/card/${Q2.id}`, {
          display: "scalar",
        });
        ...
        await findByText(DASHBOARD_NAME);
        clickScalarCardTitle(Q2.name);
      });

      test('should result in a correct query result', async () => {
    ...
    test.describe("from a dashcard multiscalar legend", () => {
      ...
      test.beforeEach(async () => {
        await createQuestionAndDashboard({
          ...
          await findByText(CARD_NAME).click();
          cy.wait("@cardQuery");
        });
      });
    ...
    test.describe("saved visualizations", () => {
      test('should respect visualization type when entering a question from a dashboard (metabase#13415)', async () => {
        ...
        await createQuestionAndDashboard({
          ...
          visitDashboard(dashboard_id);
          await findByText(QUESTION_NAME).click();
          await findByText("Category is Doohickey");
          await findByText("177"); // Doohickeys for 2016
        });
      });
    });
  });
...
async function clickScalarCardTitle(card_name) {
  await findByTestId("scalar-title").contains(card_name).click();
}

async function addCardToNewDashboard(dashboard_name, card_id) {
  await createDashboard({ name: dashboard_name }).then(
    async ({ body: { id: dashboard_id } }) => {
      await addOrUpdateDashboardCard({ card_id, dashboard_id });
      visitDashboard(dashboard_id);
    },
  );
}
