...
test.describe("scenarios > filters > sql filters > field filter > Date", () => {
  ...
  test('when set through the filter widget', async ({ page }) => {
    for (const [subType, { value, representativeResult }] of dateFilters) {
...
async function openDateFilterPicker(isFilterRequired) {
...
async function dateFilterSelector({
  filterType,
  filterValue,
  isFilterRequired = false,
} = {}) {
  await openDateFilterPicker(isFilterRequired);
