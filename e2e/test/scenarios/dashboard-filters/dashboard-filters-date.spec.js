...
test.describe("scenarios > dashboard > filters > date", () => {
  ...
  test("should show sub-day resolutions in relative date filter (metabase#6660)", async ({ page }) => {
    visitDashboard(1);
  ...
  test("correctly serializes exclude filter on non-English locales (metabase#29122)", async ({ page }) => {
...
async function dateFilterSelector({ filterType, filterValue } = {}) {
  switch (filterType) {
    case "Month and Year":
      await DateFilter.setMonthAndYear(filterValue);
      break;

    case "Quarter and Year":
      await DateFilter.setQuarterAndYear(filterValue);
      break;

    case "Single Date":
      await DateFilter.setSingleDate(filterValue);
      await cy.findByText("Update filter").click();
      break;

    case "Date Range":
      await DateFilter.setDateRange(filterValue);
      await cy.findByText("Update filter").click();
      break;

    case "Relative Date":
      await DateFilter.setRelativeDate(filterValue);
      break;

    case "All Options":
      await DateFilter.setAdHocFilter(filterValue);
      break;
...
async function clearFilter(index) {
  await filterWidget().eq(index).find(".Icon-close").click();
  await cy.wait("@dashcardQuery1");
}
