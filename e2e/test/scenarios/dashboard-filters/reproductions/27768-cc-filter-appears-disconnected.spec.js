...
describe.skip("issue 27768", (() => {
  ...
  test('filter connected to custom column should visually indicate it is connected (metabase#27768)', async ({ page }) => {
    ...
    filterWidget().click();
    await page.fill('[placeholder="Enter some text"]', "Gizmo");
    ...
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    await expect(page.locator('text=Select…')).not.toBeVisible();
    // eslint-disable-next-line no-unscoped-text-selectors -- deprecated usage
    await expect(page.locator('text=Column to filter on').locator('text=Product.CCategory')).toBeVisible();
  });
}));

async function getFilterOptions(page, filterName) {
  await page.click(`text=${filterName} >> .Icon-gear`);
}
